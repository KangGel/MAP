...
def find_closest(location, centroids):
    ...
    def helper(item):
        return distance(location,item)
    return min(centroids, key=helper)
    # END Question 3
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < 10:
        old_centroids = centroids
        clusters = group_by_centroid(restaurants,old_centroids)
        temp = []
        for cluster in clusters:
            temp.append(find_centroid(cluster))
        centroids = temp
        n += 1
    return centroids
    # END Question 6
...
def find_predictor(user, restaurants, feature_fn):
    ...
    Sxy = sum([(y - mean(ys))*(x - mean(xs)) for y,x in zip(ys,xs)])
    b = Sxy / Sxx
    a = mean(ys) - b * mean(xs)
...
def best_predictor(user, restaurants, feature_fns):
    ...
    def helper(lst1,lst2):
        for feature_fn in feature_fns:
            ...
            lst1.append(predictor),lst2.append(r_squared)
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        if restaurant in reviewed:
            dic[restaurant_name(restaurant)] = user_rating(user,restaurant_name(restaurant))
        else:
            dic[restaurant_name(restaurant)] = predictor(restaurant)
    return dic
    # END Question 9
